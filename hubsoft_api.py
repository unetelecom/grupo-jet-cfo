...
class HubsoftAPI:
    ...
    # Variações de endpoint OAuth que o Hubsoft pode usar
    OAUTH_PATHS = [
        "/oauth/token",
        "/api/oauth/token",
        "/oauth/access-token",
    ...
    def autenticar(self):
        """
        Tenta autenticar com múltiplos formatos:
        1. form-urlencoded (padrão OAuth2)
        2. JSON (fallback)
        ...
        for path in self.OAUTH_PATHS:
            ...
            # Tenta form-urlencoded primeiro (padrão OAuth2)
            for content_type, kwargs in [
                ("application/x-www-form-urlencoded", {"data": body}),
                ("application/json",                  {"json": body}),
            ]:
                try:
                    self.s.headers["Content-Type"] = content_type
                    r = self.s.post(url, timeout=20, **kwargs)
                    if r.status_code == 405:
                        errors.append(f"{path} ({content_type[:10]}): 405")
                        continue
                    if r.status_code == 404:
                        errors.append(f"{path}: 404")
                        break  # endpoint não existe, tenta próximo
                    if r.status_code not in (200, 201):
                        errors.append(f"{path} ({content_type[:10]}): {r.status_code} — {r.text[:100]}")
                        continue
                    data = r.json()
                    tok  = data.get("access_token") or data.get("token")
                    if not tok:
                        errors.append(f"{path}: sem token na resposta: {data}")
                        continue
                    # Sucesso!
                    self.token = tok
                    ...
                    errors.append(f"{path}: ConnectionError — {e}")
                    break
                except Exception as e:
                    errors.append(f"{path}: {type(e).__name__} — {e}")

        raise ConnectionError(
            "Falha na autenticação Hubsoft. Erros:\n" + "\n".join(f"  • {e}" for e in errors)
        )
    ...
    # ══════════════════════════════════════════════════════════════════
    # IMPORTAR TUDO — método principal para o app
    # ══════════════════════════════════════════════════════════════════
    def importar_tudo(self, mes=None):
        ...
        # Cobranças por vencimento do mês
        cob_mes = self.get_cobrancas(d_ini, d_fim, tipo_data="vencimento")

        # Recebimentos reais do mês (por data de pagamento)
        ...
        # rec_df: faturamento (equivale à planilha Hubsoft)
        rec_df = pd.DataFrame()
        if not cob_mes.empty:
            rec_df = cob_mes.rename(columns={"nome_cliente":"nome_razaosocial",
                                             "id_cobranca":"id_cobranca"}).copy()
            rec_df["__val"]   = pd.to_numeric(rec_df["valor"], errors="coerce").fillna(0)
            rec_df["__venc"]  = rec_df["data_vencimento"]
            rec_df["__nome"]  = rec_df.get("nome_razaosocial", pd.Series()).fillna("").astype(str)
            rec_df["__pago"]  = rec_df["status"] == "PAGO"
            rec_df["__nome_c"]= rec_df["__nome"]

        # rec_recebidos: recebidos reais (equivale ao extrato OFX)
        rec_recebidos = pd.DataFrame()
        if not cob_rec.empty:
            rec_recebidos = pd.DataFrame({
                "__pagante": cob_rec.get("nome_cliente", pd.Series(dtype=str)).fillna(""),
                "__val":     pd.to_numeric(cob_rec["valor"], errors="coerce").fillna(0),
